...
export function EditOneTab() {
    ...
    const [input, setInput] = useState<Branch>({ id: "0", name: "" })
    ...
    let editOne = async () => {
        ...
        if (res.err) { return toast({ variant: "destructive", title: "Uh oh! Something went wrong.", description: "Sorry, the can't update data!" }) }
    ...
    let getOne = async () => {
        ...
        if (res.err) { return toast({ variant: "destructive", title: "Uh oh! Something went wrong.", description: "Sorry, the can't get data!" }) }
        setInput(res?.arr || []);
        setLoad(false);
    }
            ...
            {!load &&
                <div>
                    <div key={input?.id as Key} className="grid gap-4 grid-cols-1 mb-2">
                        <Label >Nomi: </Label>
                        <Input placeholder="Nomi ... " type="text" value={input?.name} name="name" onChange={onChange} />
                    </div>
                    <div>
