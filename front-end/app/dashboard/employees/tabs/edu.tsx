...
export default function EduTab() {
    ...
    let getList = async () => {
        setLoad(true);
        const res = (await getRequest(`employees/edu?id=${params.id}`,))
        if (res.err)
    ...
    let addNew = async () => {
        const res = (await postRequest("employees/edu", { ...input }))
        if (res.err)
    ...
    let delOne = async (id: any) => {
        const res = (await deleteRequest(`employees/edu?id=${id}`))
        if (res.err)
            return toast({ variant: "destructive", title: "Uh oh! Something went wrong.", description: "Sorry, the name should not be empty!" })
        getList()
    }
                ...
                <div className="w-full">
                    ...
                    <div className="grid grid-cols-6 gap-4 w-full">
                        <div className="col-span-2" >
                            <Input type="text" placeholder="Oliygoh nomi" name="name" value={input?.name} onChange={onChange} />
                        </div>
                        <div className="col-span-2" >
                            <Input type="text" placeholder="Diplom raqam" name="num" value={input?.num} onChange={onChange} />
                        </div>
                    </div>
